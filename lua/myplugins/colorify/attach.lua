local fn = vim.fn
local api = vim.api
local ns = require("myplugins.colorify.state").ns

local get_extmarks = api.nvim_buf_get_extmarks
local methods = require "myplugins.colorify.methods"
...
return function(conf, buf, event)
  ...
  if event == "TextChangedI" then
    ...
    if conf.highlight.hex then
      methods.hex(conf, buf, cur_linenr, api.nvim_get_current_line())
    end

    if conf.highlight.lspvars then
      methods.lsp_var(conf, buf, cur_linenr)
    end
    return
  end
  ...
  if conf.highlight.hex then
    for i, str in ipairs(lines) do
      methods.hex(conf, buf, min + i - 1, str)
    end
  end
